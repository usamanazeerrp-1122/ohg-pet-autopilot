...
const OHG_PUBLISHED = ['beagle breed','boxer breed','bulldog breed','german shepherd','rottweiler','pomeranian','persian cat','ragdoll cat','deadly pet diseases','pet first aid','pet vaccines','toxic plants','zoonosis','kitten planner','pet food faq','cat grooming','pet hygiene','osteoarthritis','paw scratches','harness training','dog grooming accessories','pet grooming products'];
const ZOETIS_GAPS = ['why is my dog itching constantly','dog separation anxiety home treatment','how to potty train a puppy fast','dog arthritis early warning signs','why is my cat throwing up every day','signs of heartworm in dogs USA','flea and tick prevention dogs 2025','dog seasonal allergies treatment at home','cat urinary tract infection symptoms','puppy first vet visit what to expect','how to brush dog teeth at home','dog ear infection natural remedy','cat dental disease treatment','pancreatitis in dogs home care','dog food allergies vs intolerances','cat hairball remedies that work','dog hot spots treatment at home','kennel cough in dogs recovery','cat hyperthyroidism signs and diet','dog hip dysplasia exercises','reverse sneezing in dogs causes','cat asthma attack signs','dog anal gland problems relief','leptospirosis in dogs prevention','cat ringworm treatment home','dog epilepsy management','pet diabetes warning signs','how to trim dog nails safely','cat litter box avoidance reasons','dog vomiting yellow bile causes','cat overgrooming stress signs','dog skin rash treatment','parvo in vaccinated puppies','cat chronic kidney disease diet','dog congestive heart failure signs','cat eye discharge colors meaning','dog constipation home remedy','feline leukemia prevention','canine distemper vaccine schedule','cat scratch disease humans','dog ACL injury recovery','senior dog care guide 2025','cat arthritis pain management','dog cherry eye treatment cost','pet insurance worth it 2025','how to give liquid medicine to cat','dog cognitive dysfunction pacing','cat hyperthyroidism weight loss','dog liver disease diet','cat bladder stones symptoms','dog bloat GDV emergency signs','pet dental cleaning cost USA','dog mange home treatment','puppy socialization critical age','indoor cat enrichment ideas','adult dog crate training','how to stop dog barking','cat scratching furniture fix','loose leash walking training','best family dog breeds USA 2025','low shedding dogs for allergies','hypoallergenic cat breeds guide','best cats for first time owners','small dog breeds apartments','senior cat care tips 2025','how often to bathe a dog','cat grooming at home guide','dog nail grinding vs clipping','best food for senior dogs joints','raw diet for dogs risks','grain free dog food heart link','how much to feed a cat chart','puppy feeding schedule by age','wet vs dry cat food comparison','pet safe household cleaners list','toxic plants for cats full list','foods dogs cannot eat complete','easter lily cat toxicity','xylitol poisoning dogs symptoms','chocolate toxicity dogs amount','dog first aid kit essentials','how to perform CPR on a dog','pet emergency preparedness plan','moving with pets checklist','traveling with cats by car tips','flying with pets airline rules 2025','dog park safety rules guide','pet friendly hotels USA tips','hiking with dogs safety tips','dog swimming pool safety','camping with dogs gear list','beach safety tips for dogs','cat separation anxiety solutions','dog thunder phobia treatment','how to introduce new puppy to old dog','baby and dog introduction tips','dog resource guarding treatment','cat aggression causes fix','dog destructive chewing solutions','cat midnight zoomies fix','how to tell if dog is in pain','cat hiding behavior meaning','dog panting at night causes','cat not eating 2 days causes','dog drinking too much water causes','cat drinking more water signs','dog sudden weight loss causes','cat sudden weight gain reasons','dog bad breath home treatment','cat bad breath dental causes','dog scooting carpet fix','cat sneezing constantly causes','dog head shaking ear mites','dog limping front leg diagnosis','dog nose dry cracked causes','dog swollen belly emergency','dog eye discharge colors meaning','cat third eyelid showing causes','puppy teething relief tips','how to introduce cats properly','dog car sickness prevention','cat indoor safety checklist'];

let S = {currentTopic:null,currentArticle:null,history:[],cycleStart:null,totalCycles:0,published:0,pending:0,lastScan:null,logs:[{ts:'INIT',type:'ok',msg:'OHG Command Center ready'}]};
let timerInterval = null;

function init() {
  loadState(); startClock();
  if (S.cycleStart) restoreTimer();
  updateUI(); renderLog(); checkConn();
  // Auto-refresh page data every 30s
...
function loadState() {
  try { const d = JSON.parse(localStorage.getItem('ohg_content')||'{}'); Object.assign(S,d); addLog('info','Loaded — '+S.history.length+' topics'); } catch(e) { addLog('info','Fresh start'); }
}
function resetAll() {
  if (!confirm('Reset all CM data?')) return;
  ...
  document.getElementById('topic-main').innerHTML = '<div class="empty">🐾 No topic yet.<br>Click <span class="em-cta">▶ Run content cycle</span></div>';
...
function tickTimer() {
  ...
  if(rem===0){clearInterval(timerInterval);addLog('ok','Cycle complete');}
}
...
function updateUI() {
  set('s-total',S.totalCycles); set('s-art',S.history.filter(h=>h.hasArticle).length);
  set('s-pend',S.pending); set('s-pub',S.published);
  const rem=Math.max(0,ZOETIS_GAPS.length-S.totalCycles);
  set('cm-gap-info','gaps remaining: '+rem+' of '+ZOETIS_GAPS.length);
  ...
  if(S.currentTopic&&S.currentTopic.status!=='published') document.getElementById('pub-btn').style.display='block';
  if(S.currentArticle) { document.getElementById('gen-btn').style.display='block'; document.getElementById('gen-btn').textContent='↻ Regenerate article'; }
}
...
function renderCurrentTopic() {
  ...
  document.getElementById('topic-main').innerHTML=
    ...
    '<div style="font-size:10px;color:var(--muted);font-family:DM Mono,monospace;margin-bottom:6px;display:flex;gap:7px;flex-wrap:wrap">'+
...
function renderHistory() {
  ...
  area.innerHTML='<div style="max-height:180px;overflow-y:auto">'+S.history.slice().reverse().map((h,i)=>
    '<div class="hrow"><span class="hnum">#'+(S.history.length-i)+'</span><div style="flex:1"><div class="htitle">'+esc(h.title)+'</div>'+
    '<div class="hmeta">'+(h.date||'')+(h.hasArticle?' · article ready':'')+'</div></div>'+
    '<span class="hstatus '+(h.status==='published'?'pub':'p')+'">'+(h.status==='published'?'✓':'●')+'</span></div>'
  ).join('')+'</div>';
}
...
async function callClaudeWithRetry(messages,useSearch,retries=2){
  for(let i=0;i<=retries;i++){try{return await callClaude(messages,useSearch);}catch(e){if(e.message.includes('429')&&i<retries){addLog('warn','Rate limit — waiting 30s...');await new Promise(r=>setTimeout(r,30000));}else throw e;}}
}
function startGenCooldown(){
  const btn=document.getElementById('gen-btn');let secs=20;btn.disabled=true;
  const tick=setInterval(()=>{btn.textContent='⏳ '+secs+'s cooldown';secs--;if(secs<0){clearInterval(tick);btn.disabled=false;btn.textContent='✦ Generate article';}},1000);
}
...
async function runCycle() {
  ...
  document.getElementById('topic-main').innerHTML='<div class="empty"><span class="spin" style="width:18px;height:18px;border-width:3px;display:block;margin:0 auto 8px"></span><span style="color:var(--blue)">Searching USA pet trends + Zoetis gaps...</span></div>';
  ...
  try {
    ...
    document.getElementById('gen-btn').style.display='block';startGenCooldown();
    ...
    document.getElementById('topic-main').innerHTML='<div class="err-box"><div style="font-weight:600;margin-bottom:4px">Cycle error</div>'+esc(e.message)+'<div class="err-fix">Check: Railway running? CLAUDE_KEY set? /claude-proxy deployed?</div></div>';
  }
...
async function generateArticle() {
  ...
  addLog('info','Generating: '+S.currentTopic.title);
  document.getElementById('article-area').innerHTML='<div class="cm-topic" style="margin-top:8px"><div class="empty"><span class="spin" style="width:18px;height:18px;border-width:3px;display:block;margin:0 auto 8px"></span><span style="color:var(--blue)">Writing 900-1100 words · Schema · FAQ · OHG brand system...</span></div></div>';
  const t=S.currentTopic;
  const prompt='You are the OHG content writer. Generate complete standalone HTML page with OHG Premium Brand System.\n\nTOPIC: "'+t.title+'"\nPRIMARY: '+t.keywords[0]+'\nSECONDARY: '+(t.keywords.slice(1)||[]).join(', ')+'\nDATE: '+new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})+'\n\nFONTS: DM Serif Display + DM Sans\nCSS: --g1:#0b1f13;--g2:#145238;--g3:#1a6b4a;--g4:#2d9c6e;--g5:#e8f5ef;--gold:#d4a04a;--cream:#fdfaf6;--red:#dc2626\n\nSECTIONS: VetBanner→StickyNav→Hero→Disclaimer→Table→Content→GlanceBox→Cards→SymptomGrid→VetAlert→MidCTA→EmergencyGrid→FAQ→ResourceGrid→FinalCTA→FooterDisclaimer\n\nINTERNAL LINKS: /dog-paw-scanner/ /pet-vaccine-tracker/ /pet-first-aid-kit-checklist/ /pet-safety-hub/ onehealthglobe.com\n\nSEO: JSON-LD BreadcrumbList+Article+FAQPage, 50-60 char title, 145-155 char desc, 900-1100 words, EEAT phrases, 3-5 FAQ H4s\n\nOutput ONLY complete HTML. No preamble.';
  try {
    const html=await callClaudeWithRetry([{role:'user',content:prompt}],false);
    S.currentArticle=html;
    const wordCount=html.replace(/<[^>]+>/g,'').split(/\s+/).filter(Boolean).length;
    addLog('ok','Article: ~'+wordCount+' words');
    ...
    saveState();updateUI();
    document.getElementById('article-area').innerHTML=
      '<div class="article-card"><div style="font-size:13px;font-weight:600;margin-bottom:7px">'+esc(t.title)+'</div>'+
      '<div style="font-size:10px;color:var(--muted);font-family:DM Mono,monospace;margin-bottom:8px;display:flex;gap:7px;flex-wrap:wrap">'+
      '<span style="background:var(--bg);padding:2px 6px;border-radius:3px">~'+wordCount+' words</span>'+
      '<span style="background:var(--bg);padding:2px 6px;border-radius:3px">schema ✓</span>'+
      '<span style="background:var(--bg);padding:2px 6px;border-radius:3px">FAQ ✓</span>'+
      '<span style="background:var(--bg);padding:2px 6px;border-radius:3px">internal links ✓</span>'+
      '<span style="background:var(--bg);padding:2px 6px;border-radius:3px">OHG brand ✓</span></div>'+
      '<div style="display:flex;gap:6px;margin-bottom:8px">'+
      '<button class="btn sm blue-btn" onclick="downloadArticle()">↓ Download HTML</button>'+
      '<button class="btn sm" onclick="copyArticle()">⊕ Copy</button></div>'+
  ...
  btn.disabled=false;btn.textContent='↻ Regenerate article';
}
...
function downloadArticle(){
  ...
  addLog('ok','Downloaded HTML');
}
function copyArticle(){
  if(!S.currentArticle) return;
  navigator.clipboard.writeText(S.currentArticle).then(()=>{addLog('ok','Copied to clipboard');alert('✓ Copied! Paste into WordPress Code Editor.');}).catch(()=>addLog('warn','Clipboard failed — use Download'));
}
...
async function scanSite(){
  ...
  try {
    ...
    text.split('\n').filter(l=>l.trim()).slice(0,10).forEach(l=>addLog('info',l.trim().substring(0,100)));
    const now=new Date().toLocaleString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
    S.lastScan=now;set('scan-info','last scan: '+now);saveState();
